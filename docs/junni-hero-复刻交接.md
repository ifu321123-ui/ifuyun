...
- 是 **卡片正反翻面（180°）** + `backface-visibility:hidden`（轴向后改为 `rotateX(-180)` 上下翻，见坑 11）
...
- 每格延迟**自己按「到网格中心的距离」计算**，实现 from-center 扩散：
  ...
  rotateX = -(local * 180)   // 轴向后改为 rotateX 上下翻、负角，见坑 11
  ```
...
section.junni-hero-stage            高 = JUNNI_STAGE_VH*100vh（滚动行程）
 └ div.junni-hero-scene             position:sticky; top:0; height:100vh; overflow:hidden
    └ div.junni-grid-perspective    perspective:1200px ★唯一共享透视（整体可 display:none）
       └ div.junni-grid             preserve-3d; translateZ(-20px)（写死纵深，不接鼠标）; grid 6×6+gap
          └ 36× div.junni-cell      静止底座+鼠标命中靶：preserve-3d; pointer-events:auto;（mouseenter/leave 绑这里，见坑 12）
             └ div.junni-card       SCROLL 层：JS 写 rotateX(-(local*180)deg); 无 transition; preserve-3d; pointer-events:none（继承下传，旋转链对鼠标隐形）
                └ div.junni-card-flip  HOVER 层：.is-hovered→rotateX(-180); transition .5s; preserve-3d
...
rotateX = -(local * 180)                                  // 写到 .junni-card（绕 X 轴上下翻，负角=向上翻，对齐原站 rotateX(-180deg)）
```
> 2026-06-15 轴向修正：翻转轴由 `rotateY` 改为 `rotateX`（上下百叶窗翻面），背面 `.junni-face--back` 与 Hover `.is-hovered` 同步改为 `rotateX(180deg)` / `rotateX(-180deg)`；180° 绿底终点与现有无缝衔接保持不变。鼠标全局随动公式已随坑 10 一并废弃（见下）。
...
7. **逐格 Hover 翻面用「双层」拆分（修正 A）**：scroll 翻转角由 JS 每帧覆写在 `.junni-card`（**无 transition**，保证 scrub 跟手）；Hover 的 180° 增量单独挂在内层 `.junni-card-flip` 上，靠 CSS `transition: transform 0.5s cubic-bezier(0.25,1,0.5,1)` 平滑。两层都 `preserve-3d`、同轴 `rotateX` 自然叠加，互不打架（轴向修正见坑 11）。仅 `progress<=FLIP_START`（首屏静止）时 `mouseenter` 加 `.is-hovered`；一旦滚动越过 `FLIP_START`，`apply()` 内 `clearHover()` 撤掉所有悬浮翻面。事件监听绑在底座 `.junni-cell`（防抽搐解耦见坑 12）。
...
- [x] ~~鼠标随动 3D 浮动~~（已废弃并移除，原站静止首屏无整体倾斜，见 4.5 / 坑 10）
- [x] **轴向修正为 rotateX 上下翻面**（JS `-(local*180)` + 背面/Hover 同步 rotateX，180° 绿底终点不变，2026-06-15）
- [x] **逐格 Hover 独立翻面**（首屏静止时 mouseenter→rotateX(-180)，双层叠加见坑 7）
- [x] **根除 Hover 鬼畜抽搐**（事件绑底座 `.junni-cell` + 旋转链 `.junni-card` 继承 `pointer-events:none`，见坑 12，2026-06-15）
